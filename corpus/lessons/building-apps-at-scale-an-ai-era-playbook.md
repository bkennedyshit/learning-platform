...
> *Practical field guide for building real software as an AI-assisted developer. Not a textbook. Not a course. A "what's actually true in 2026 and where do I learn each piece" map that points across the [Learning curriculum](00---09---Learning-Index).*
...
- [1.9 - Docker & Containers](1.9---Docker-&-Containers) — the chapter for hands-on Docker.
- [1.10 - Operating Systems Essentials](1.10---Operating-Systems-Essentials) — context for what containers are isolating *from*.
- [1.16 - Distributed Systems & Multi-GPU Training](1.16---Distributed-Systems-&-Multi-GPU-Training) — when Docker stops being enough.
...
- **Networking layer** — UDP / WebRTC / WebTransport / WebSockets. ([Track 04](Subject_Plan) covers netcode patterns.)
...
For VR/spatial computing the *exact same shape* applies, just with sub-20ms hand-tracking sync added on top. We covered it in [24.7 - Networking, Avatars & Multi-User Spaces](24.7---Networking,-Avatars-&-Multi-User-Spaces) and the parallel patterns appear in [22.5 — ROS 2 & DDS](22.5---ROS-2-&-Middleware---Nodes,-Topics,-Services,-Actions,-DDS) (the same publish/subscribe model robotics uses for multi-node coordination).
...
- **Photon Fusion / Mirror / Netcode for GameObjects / Unreal Replication** — see [Subject_Plan](Subject_Plan) and [24.7 - Networking, Avatars & Multi-User Spaces](24.7---Networking,-Avatars-&-Multi-User-Spaces).
...
|---|---|---|
| **Foundation** | Operating systems, processes, files, sockets | [1.10 - Operating Systems Essentials](1.10---Operating-Systems-Essentials) |
| | Networks: TCP/UDP, HTTP/2/3, DNS, TLS | [1.11 - Computer Networks Essentials](1.11---Computer-Networks-Essentials) |
| | Concurrency: threads, async, GIL, message passing | [1.4 - Concurrency - asyncio, threading, multiprocessing & the GIL](1.4---Concurrency---asyncio,-threading,-multiprocessing-&-the-GIL), [1.14 - Concurrency Models & Patterns](1.14---Concurrency-Models-&-Patterns) |
| | Computer architecture (CPU, cache, RAM, NVMe, GPU) | [1.12 - Computer Architecture - Performance Intuition](1.12---Computer-Architecture---Performance-Intuition), [21.6 - Motherboards & Computer Architecture - CPU, RAM, Chipset, PCIe, UEFI](21.6---Motherboards-&-Computer-Architecture---CPU,-RAM,-Chipset,-PCIe,-UEFI) |
| **Data** | SQL fluency (joins, indexes, transactions, ACID) | [Subject_Plan](Subject_Plan) |
| | Schema design, migrations, query plans | [Subject_Plan](Subject_Plan) |
| **App layer** | Frontend frameworks (React/Next, Angular, Svelte/Solid) | [Subject_Plan](Subject_Plan), [Subject_Plan](Subject_Plan) |
| | Backend frameworks (FastAPI, Hono/NestJS, Rails…) | [Subject_Plan](Subject_Plan) + [Subject_Plan](Subject_Plan) |
| | Cross-platform (Flutter, PyQt, Tauri) | [Subject_Plan](Subject_Plan) |
| **Distribution** | Docker fundamentals | [1.9 - Docker & Containers](1.9---Docker-&-Containers) |
| | Distributed systems patterns (replication, consensus, eventual consistency) | [1.16 - Distributed Systems & Multi-GPU Training](1.16---Distributed-Systems-&-Multi-GPU-Training) |
| | Pub/sub & middleware (DDS, NATS, Kafka, Redis Streams) | [22.5 - ROS 2 & Middleware - Nodes, Topics, Services, Actions, DDS](22.5---ROS-2-&-Middleware---Nodes,-Topics,-Services,-Actions,-DDS) (same patterns, different domain) |
| **Multi-user** | Real-time networking, presence, conflict resolution | [Subject_Plan](Subject_Plan), [24.7 - Networking, Avatars & Multi-User Spaces](24.7---Networking,-Avatars-&-Multi-User-Spaces) |
| **AI integration** | LLM inference, agents, RAG, multimodal | [Subject_Plan](Subject_Plan), [Subject_Plan](Subject_Plan) |
| **Cybersecurity** | Threat modeling, OWASP Top 10, AppSec | [Subject_Plan](Subject_Plan) (8 chapters) |
| **DevOps / SRE** | CI/CD, observability, incident response, SLOs | [Subject_Plan](Subject_Plan) (8 chapters) |
| **System design** | Capacity planning, caching, sharding, queuing | [Subject_Plan](Subject_Plan) (8 chapters) |
| **Cloud platforms** | Provider survey, compute / storage / IAM, indie stack | [Subject_Plan](Subject_Plan) (8 chapters) |
...
- **Foundations:** [1.7 - Shell, Terminal & Cross-Platform CLI](1.7---Shell,-Terminal-&-Cross-Platform-CLI), [1.8 - Git & Version Control](1.8---Git-&-Version-Control), [1.9 - Docker & Containers](1.9---Docker-&-Containers).
- **Data fluency:** [Subject_Plan](Subject_Plan) (chapters 1–4 minimum).
- **One backend framework + one frontend framework:** [Subject_Plan](Subject_Plan) + [Subject_Plan](Subject_Plan) (8.1 React/Next or 8.5 Flutter).
...
- [1.16 - Distributed Systems & Multi-GPU Training](1.16---Distributed-Systems-&-Multi-GPU-Training) for the core mental models.
- [22.5 - ROS 2 & Middleware - Nodes, Topics, Services, Actions, DDS](22.5---ROS-2-&-Middleware---Nodes,-Topics,-Services,-Actions,-DDS) — the *same* pub/sub primitives appear in robotics; understanding them in one domain transfers fully to the other.
...
| Gap | Track | Status |
|---|---|---|
| **Cybersecurity** | [Track 25 — Cybersecurity](Subject_Plan) | 🟡 Skeleton (8 ch, ~140 KB). Threat modeling, OWASP Top 10:2025, modern auth (WebAuthn / passkeys), cryptography (incl. NIST PQC: ML-KEM / ML-DSA / SLH-DSA), secure SDLC (Sigstore / SLSA), network sec, cloud/container sec, AI security (OWASP LLM Top 10:2025). |
| **DevOps / SRE / Observability** | [Track 26 — DevOps & SRE](Subject_Plan) | 🟡 Skeleton (8 ch, ~135 KB). CI/CD, IaC (Terraform / OpenTofu / Pulumi), containers + GitOps (Argo CD / Flux), OpenTelemetry observability, SLOs / error budgets / incident response, chaos engineering, deployment strategies, FinOps. |
| **System Design / Microservices / API Design** | [Track 27 — System Design & Distributed Architecture](Subject_Plan) | 🟡 Skeleton (8 ch, ~130 KB). CAP / PACELC fundamentals, caching + CDN + edge, databases at scale, message queues + event-driven (Kafka / NATS), API design (REST / GraphQL / gRPC), microservices + service mesh, real-time + CRDTs, capacity planning. |
| **Cloud Platforms** | [Track 28 — Cloud Platforms (AWS / GCP / Azure / Cloudflare)](Subject_Plan) | 🟡 Skeleton (8 ch, ~135 KB). 2026 provider landscape, compute primitives (incl. GPU clouds), storage + databases, networking + DNS + CDN, IAM + org structure, FinOps cost optimization, multi-cloud + edge + lock-in, the indie & solo stack. |
...
- Master curriculum index: [00 - 09 - Learning Index](00---09---Learning-Index)
- Practice / drill workflow: [HOW_TO_USE_PRACTICE](HOW_TO_USE_PRACTICE)
- Foundations: [Subject_Plan](Subject_Plan), [Subject_Plan](Subject_Plan), [Subject_Plan](Subject_Plan)
- App architecture: [Subject_Plan](Subject_Plan)
- Multi-user / netcode: [Subject_Plan](Subject_Plan), [24.7 - Networking, Avatars & Multi-User Spaces](24.7---Networking,-Avatars-&-Multi-User-Spaces)
- Distributed-systems patterns: [1.16 - Distributed Systems & Multi-GPU Training](1.16---Distributed-Systems-&-Multi-GPU-Training), [22.5 - ROS 2 & Middleware - Nodes, Topics, Services, Actions, DDS](22.5---ROS-2-&-Middleware---Nodes,-Topics,-Services,-Actions,-DDS)
- AI integration: [Subject_Plan](Subject_Plan), [Subject_Plan](Subject_Plan)
- Hardware-aware perf intuition: [1.12 - Computer Architecture - Performance Intuition](1.12---Computer-Architecture---Performance-Intuition), [21.6 - Motherboards & Computer Architecture - CPU, RAM, Chipset, PCIe, UEFI](21.6---Motherboards-&-Computer-Architecture---CPU,-RAM,-Chipset,-PCIe,-UEFI)
- Productization angle (your architect-into-business story): [20.8 - Pipelines, Interop & Productization - From Architecture to Business](20.8---Pipelines,-Interop-&-Productization---From-Architecture-to-Business), [24.8 - VR as a Business - Productization, Distribution, Monetization](24.8---VR-as-a-Business---Productization,-Distribution,-Monetization), [23.7 - Holography in Architecture & Engineering Visualization](23.7---Holography-in-Architecture-&-Engineering-Visualization)
- **Production-engineering arc (the four tracks this playbook called for):**
    - Cybersecurity: [Subject_Plan](Subject_Plan) · [LEARNING_PATH](LEARNING_PATH) · [README](README)
    - DevOps & SRE: [Subject_Plan](Subject_Plan) · [LEARNING_PATH](LEARNING_PATH) · [README](README)
    - System Design: [Subject_Plan](Subject_Plan) · [LEARNING_PATH](LEARNING_PATH) · [README](README)
    - Cloud Platforms: [Subject_Plan](Subject_Plan) · [LEARNING_PATH](LEARNING_PATH) · [README](README)
