...
Walk every chapter `.md` file under 10 - Learning App Material/Subjects/,
extract every inline <svg>...</svg> block into a separate `.svg` file in a
sibling `_svgs/` folder, and replace the inline block in the chapter with
an Obsidian wikilink reference like ![[02.1-fig1.svg|500]]

Uses safe_print to prevent stdout crashes on Windows terminals.
"""
...
ROOT = Path(r"C:\Obsidian Vault\Bill's Vault\05-Knowledge_Foundation\10 - Learning App Material\Subjects")
...
def chapter_num_from_filename(fname: str) -> str:
    """'02.1 - Number Sense.md' -> '02.1', falls back to filename stem."""
    m = re.match(r"^(\d+\.\d+)", fname)
    return m.group(1) if m else Path(fname).stem

def extract_width(svg_text: str) -> int:
    """Return the first max-width:Npx hint, defaulting to 500."""
...
def main() -> int:
    ...
    safe_print("Starting extraction of inline SVGs from K-12 App chapters...")
    for md in ROOT.rglob("*.md"):
        ...
        if count > 0:
            ...
            safe_print(f"  {count:2d}x  {rel}")
            for n in names:
                safe_print(f"        -> _svgs/{n}")

    verb = "Would extract" if args.dry_run else "Extracted"
    safe_print(f"\nCompleted: {verb} {total_svgs} SVGs from {total_chapters} chapter files")
    return 0
