...
mission: "Master Rust's algebraic type system — enums as tagged unions, traits as zero-cost interfaces, and generics with monomorphization"
status: active
tags: [rust, enums, traits, generics, pattern-matching, algebraic-types, monomorphization, trait-objects]
type: chapter
chapter: 11.3
track: 14
prev: "[[11.2 - Ownership, Borrowing & Lifetimes]]"
next: "[[11.4 - Error Handling - Result, Option, ? Operator]]"
---
...
- **Next**: [[11.4 - Error Handling - Result, Option, ? Operator]] — Result<T,E> is just an enum!
- **Patterns chapter**: [[11.7 - Idiomatic Rust - Patterns, Anti-patterns & Performance]] — Newtype, Builder, Typestate
- **Python protocols**: [[08.3 - OOP, Data Models & Pythonic Idioms]] — Traits are Rust's version of Python's Protocol
- **Game dev ECS**: [[11.8 - Production Rust - WebAssembly, FFI, Embedded & Game Dev (Bevy)]] — Bevy uses traits extensively
...
fn main() {
    ...
    let x: Matrix<f64, 2, 2> = Matrix::from_array([[1.0, 2.0], [3.0, 4.0]]);
    let y: Matrix<f64, 2, 2> = Matrix::from_array([[5.0, 6.0], [7.0, 8.0]]);
    let sum = x + y;
    println!("X + Y =\n{sum}");
