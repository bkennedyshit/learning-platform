---
tags: [electrodynamics, magnetostatics, biot-savart, ampere-law, magnetic-field, vector-potential, current-density]
chapter: 7.3
prev: "[[7.2 - Laplace & Poisson Equations]]"
next: "[[7.4 - Electrodynamics - Induction & Maxwell's Equations]]"
---
...
$\mathbf{A}$ is called the **magnetic vector potential**. It is not unique — adding any gradient $\nabla\lambda$ to $\mathbf{A}$ leaves $\mathbf{B}$ unchanged (gauge freedom, explored in [[7.6 - Potential Formulations & Gauge Transformations]]).
...
where $S$ is any surface bounded by $\mathcal{C}$ (by Stokes' Theorem from [[1.7 - Green's Stokes' and Divergence Theorems]]).
...
This has the same angular structure as the electric dipole field (Definition 7.1.8 in [[7.1 - Electrostatics - Gauss's Law & Potential]]).
...
**Step 3:** Apply Stokes' Theorem (from [[1.7 - Green's Stokes' and Divergence Theorems]]) to the left side:
...
|:---|:---|:---|
| Electrostatic parallel | [[7.1 - Electrostatics - Gauss's Law & Potential]] | Structural analog: $\nabla\cdot\mathbf{E}$ ↔ $\nabla\times\mathbf{B}$ |
| Integral theorems | [[1.7 - Green's Stokes' and Divergence Theorems]] | Stokes' theorem gives Ampère's Law |
| Vector calculus | [[1.6 - Vector Fields, Div & Curl]] | Curl and divergence operations |
| Induction | [[7.4 - Electrodynamics - Induction & Maxwell's Equations]] | Time-varying $\mathbf{B}$ creates $\mathbf{E}$ |
| Gauge freedom | [[7.6 - Potential Formulations & Gauge Transformations]] | Non-uniqueness of $\mathbf{A}$ |
| Relativistic view | [[7.7 - Relativistic Electrodynamics & Four-Vectors]] | $\mathbf{B}$ is a frame-dependent aspect of $F^{\mu\nu}$ |
